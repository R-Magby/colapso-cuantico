...
__global__ void Dissipation(double *phi, double *chi, double *PI, double *A, double *B, double *alpha, double *Da, double *Db, 
    ...
    cufftDoubleComplex *u, cufftDoubleComplex *u_p1,  cufftDoubleComplex *pi, cufftDoubleComplex *dr_u,
    cufftDoubleComplex *u_ghost, cufftDoubleComplex *u_p1_ghost,  cufftDoubleComplex *pi_ghost, cufftDoubleComplex *dr_u_ghost,
    double dt, int Nr, int Nl, int Nk, int t){
int idx = threadIdx.x + blockDim.x*blockIdx.x;
      ...
      //fluctuacion tiempo tiempo
        for (int k=0 ; k < Nk ; k++){
            ...
            if(t==0){
              ...
              d_u_temp_x = ( up1_ghost[ g*Nl*Nk*Nr + l*Nk*Nr + k*Nr + idx].x * pow(radio , l) - u_ghost[ g*Nl*Nk*Nr + l*Nk*Nr + k*Nr + idx].x * pow(radio , l) )/dt ;
              d_u_temp_y = ( up1_ghost[  g*Nl*Nk*Nr + l*Nk*Nr + k*Nr + idx].y * pow(radio , l) - u_ghost[ g*Nl*Nk*Nr + l*Nk*Nr + k*Nr + idx].y * pow(radio , l) )/dt ;
            }
            /*if(idx==50 && l==17){
                printf("d_u_temp_x = %.15f  |  k = %d\n",d_u_temp_x,k);
        ...
        for (int k=0 ; k < Nk ; k++){
          //derivada radial compleja
            ...
            if(t==0){
                res = derivate_complex_nodos(u_ghost, g*Nl*Nk*Nr + l*Nk*Nr + k*Nr + idx, dr,Nr,l,0 );
                d_u_temp_x = res.x;
                d_u_temp_y = res.y;
        ...
        for (int k=0 ; k < Nk ; k++){
          //derivada temporal y radial compleja
            ...
            if(t==0){
                ...
                res = derivate_complex_nodos(up1_ghost, g*Nl*Nk*Nr + l*Nk*Nr + k*Nr + idx, dr,Nr,l,0 );
                d_u_temp_x = res.x * ( up1_ghost[ g*Nl*Nk*Nr + l*Nk*Nr + k*Nr + idx].x * pow(radio , l) - u_ghost[ g*Nl*Nk*Nr + l*Nk*Nr + k*Nr + idx].x * pow(radio , l) )/dt ;
                d_u_temp_y = res.y * ( up1_ghost[ g*Nl*Nk*Nr + l*Nk*Nr + k*Nr + idx].y * pow(radio , l) - u_ghost[ g*Nl*Nk*Nr + l*Nk*Nr + k*Nr + idx].y * pow(radio , l) )/dt ;
              }
          if (k==0){
            temp_fluct[ 2*Nr + idx] = d_u_temp_x +  d_u_temp_y;
          }
...
__global__ void stress_energy(double *SA, double *SB, double *rho, double *ja, double *phi, double *chi, double *PI, double *A, double *B, double *alpha, double *temp_array , double dr, double dt, int Nr, int Nk, int Nl, int t){
  ...
  if(idx < Nr){
    ...
    espectation_dt_phi = PI[idx]*PI[idx] + A[idx]*B[idx]*B[idx]/(alpha[idx]*alpha[idx]) * temp_array[ idx ];

    espectation_dr_phi = chi[idx]*chi[idx] + temp_array[ 1*Nl*Nr + idx ];


    espectation_dr_dt_phi = PI[idx]*chi[idx] + sqrt(A[idx])*B[idx]/alpha[idx] * temp_array[ 2*Nl*Nr + idx ];
...
__global__ void stress_energy_ghost(double *SA, double *SB, double *rho, double *ja,double *A, double *B, double *alpha, double *temp_array , double dr, double dt, int Nr, int Nk, int Nl, int t, double mu, int g){
    ...
    if(idx < Nr){
      ...
      double xPI2x , xchi2x, xpichix;
      xPI2x =  A[idx]*B[idx]*B[idx]/(alpha[idx]*alpha[idx]) *temp_array[ idx ];
      xchi2x =  temp_array[ 1*Nl*Nr + idx ];
      xpichix = sqrt(A[idx])*B[idx]/alpha[idx] *  temp_array[ 2*Nl*Nr + idx ];
      //Stress-Energy
...
__global__ void Tensor_tt(double * T_tt, double *rho, int t, int Nr){
  int idx = threadIdx.x + blockDim.x*blockIdx.x;
  if(idx<Nr){
...
printf("thread = %d , block = %d , dk = %lf ", thread , (int)ceil((float)(Nr*Nk*Nl)/thread) , dk);
  for (int t = 0 ; t < 100   ; t++ ){
    ...
    Dissipation<<<1,4>>>(   cuda_phi , cuda_chi , cuda_PI, cuda_A, cuda_B, cuda_alpha, cuda_Da, cuda_Db, cuda_K, cuda_Kb, cuda_lambda,
                            cuda_U, cuda_u_nodos, cuda_u_nodos_p1, cuda_pi, cuda_dr_u, 
                            cuda_u_ghost,  cuda_u_ghost_p1, cuda_pi_ghost, cuda_dr_u_ghost,dt,Nr,Nl,Nk,t);

    cudaDeviceSynchronize();


    Tensor_tt<<< grid_radial,bloque >>>(cuda_T_tt, cuda_rho,  t,  Nr);
    cudaDeviceSynchronize();
...
cudaMemcpy(T_tt, cuda_T_tt, Nr*Nt*sizeof(double), cudaMemcpyDeviceToHost);
/*
  for(int r=0; r<Nr; r++){
    printf("phi_%d : %.15f\n",r,phi[5*Nr + r]);
  }
