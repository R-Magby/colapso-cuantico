...
void guardar_salida_SB(double *data,int T) {
  ...
  __device__ double psi_dot(Relativity_G metrics, double *field_pi, int id_nodo, int idx){
    double temp,sym;
    double f,fm1,fm2,fp1,fp2,fm3,fp3;
    temp = 0.0;
...
__device__ void absorbente(double *R,int id_nodo_ghost, int id,double c,int planck){
  //dgdt=-1.0*c*(3.0*g[idx]-4.0*g[idx-1] + g[idx-2])/(2.0*dr);
  if(id==Nr-1){
    if(planck==0){
      //R[id] = R[id] - 1.0*c*derivate(R, id_nodo_ghost,id,0) *dt;
      //R[id] = R[id-1];  
    }
    else{
      //R[id_nodo_ghost + id] = R[ id_nodo_ghost + id] - (1.0*c*derivate(R, id_nodo_ghost,id,0));// - R[id]/(id*dr))*dt;
      //R[id] = R[id-1];
...
__global__ void Kreiss_Oliger(double *G, double *R, double epsilon,int symmetric, int g, int planck,double *alpha){
  ...
  int id_nodo_ghost =g*dim_q +l*Nr*Nk + k*Nr;
    ...
    if(r<(int)Nr){
        ...
        if (id_nodo < half_derivate_twelfth_left){
        //printf("next %d\n ",r);
        //printf("d12: %.15f | R = %.15f | r: %d\n",epsilon*twelfth_derivate(R, 0, r,symmetric,1),R[id],r);
        }
    }
    }
    else if (planck==1){
    ...
    //else if(id < 6*dim_q && r>(int)Nr/2){
        //G[id] = G[id] - 0.5*pow(dr,0)*twelfth_derivate(R, id_nodo_ghost, r,symmetric,planck);
...
__global__ void guardar_datos(double *f,double *alpha_temp, double *nodo, double *A_temp, Relativity_G metrics ,field_Classic field, field_Quantum field_Q, int t){
  int idx = threadIdx.x + blockDim.x*blockIdx.x;
  ...
  if(idx<Nr){
    f[ t*Nr + idx] = field_Q.real.psi[0*dim_q + 0*Nr*Nk + 0*Nr + id];
    alpha_temp[ t*Nr + idx] = psi_dot(metrics, field_Q.imag.pi, id_nodo_ghost ,r);
    A_temp[ t*Nr + idx] = metrics.U[idx];
    nodo[t*Nr + idx]=derivate(field_Q.real.psi,id_nodo_ghost,r,1);//field_Q.imag.psi[5*dim_q + 2*Nr*Nk + 2*Nr + id];
    ...
    if(idx == Nr-1){
      ...
      f4 = mass * mass* field_Q.real.u[id];
      printf("id : %d | r: %d | k: %d | l: %d | id_nodo : %d | id_nodo_g : %d\n",id,r,k,l,id_nodo,id_nodo_ghost);
      printf(" u: %.15f | psi: %.15f | pi: %.15f | f1:%.15f | f2 : %.15f | f3: %.15f\n",field_Q.real.u[id],field_Q.real.psi[id] , (field_Q.real.pi[id]),f1,f2,f3);
      printf(" arg: %.15f | drpsi: %.15f | lambda: %.15f |\n",l*(l+1)/radio,derivate(field_Q.real.psi,id_nodo_ghost,r,1) , derivate(field_Q.real.psi,id_nodo_ghost,Nr/2,1));

      printf(" Da: %.15f | Db: %.15f | K: %.15f | Kb:%.15f | U : %.15f \n",metrics.Da[r],metrics.Db[r] , metrics.K[r],metrics.Kb[r],metrics.U[r]);
    ...
    if (idx < Nr){
        ...
        if(idx==0 && siono==0 && g==5){
          ...
          printf("Stress_Energy en  idx : %d of g : %d:\n",idx,g); 
            ...
            printf("rho : %.15f | rho 1: %.15f | rho 2: %.15f\n",rho[idx],rho[idx+1],rho[idx+2]);
            printf("ja : %.15f | ja |: %.15f | ja 1: %.15f\n",ja[idx], ja[idx+1], ja[idx+2] );
            printf("SA : %.15f\n",SA[idx]);
            printf("SB : %.15f\n",SB[idx]);
        
            printf("dA : %.15f\n",derivate(metrics.A,0, idx, 0));
            printf("dB : %.15f\n",derivate(metrics.B,0, idx, 0));
            printf("dalpha : %.15f\n",derivate(metrics.alpha,0, idx, 0));
            printf("dDa : %.15f\n",derivate(metrics.Da,0, idx, 1));
            printf("dDb : %.15f\n",derivate(metrics.Db,0, idx, 1));
            printf("dK : %.15f\n",derivate(metrics.K,0, idx, 0));
            printf("dKb : %.15f\n",derivate(metrics.Kb,0, idx, 0));
            printf("dlambda : %.15f\n",derivate(metrics.lambda,0, idx, 1));
            printf("dU : %.15f\n",derivate(metrics.U,0, idx, 1));
    ...
    if(id < Nr){
      ...
      if(id==0){radio=dr;}
        ...
        if(id<Nr-1){
          ...
          metrics_RK.Da[id]      = -derivate(metrics.Da, 0, id, 1);
          metrics_RK.Db[id]      = -derivate(metrics.Db, 0, id, 1);
        }
        ...
        if(id<Nr-1){
          ...
          metrics_RK.K[id] =-derivate(metrics.K,0,id,0);
        }
        if(id==Nr-1){
        ...
        if(id<Nr-1){
          ...
          metrics_RK.Kb[id] =-derivate(metrics.Kb,0,id,0);
        }
        if(id==0){
        ...
        if(id<Nr-1){
          ...
          metrics_RK.U[id]      =-derivate(metrics.U,0,id,1);
          metrics_RK.lambda[id] =-derivate(metrics.lambda,0,id,1);
        }
...
__global__ void evo_fields_quantums(  field_Quantum field_Q, field_Quantum field_Q_RK, Relativity_G metrics,  int g, int s){
    ...
    if(id < 6*dim_q){
      ...
      if(r==0){
        ...
        temp[0]=-c*derivate(field_Q.real.psi,id_nodo_ghost,r,1);
        temp[0]=-c*derivate(field_Q.imag.psi,id_nodo_ghost,r,1);
      ...
      if (id==0*dim_q + 4*Nr*Nk + 4*Nr + 498){
        //printf("id : %d | r: %d | k: %d | l: %d | id_nodo : %d | id_nodo_g : %d\n",id,r,k,l,id_nodo,id_nodo_ghost);
        ...
        //printf("s: %d | g: %d | arg: %.15f | u: %.15f | lambda: %.15f |\n",s,g,l*(l+1)/radio,derivate(field_Q.real.psi,id_nodo_ghost,r,1), (metrics.lambda[r]));
      ...
      if(r<Nr-1){
        ...
        field_Q_RK.real.pi[id] = -c*derivate(field_Q.real.pi,id_nodo_ghost,r,0);
      }
      if (id==0*dim_q+Nr-1){
      ...
      if(r<Nr-1){
        ...
        field_Q_RK.imag.pi[id] = -c*derivate(field_Q.imag.pi,id_nodo_ghost,r,0);
      }      
...
__global__ void y_tilde_metrics( Relativity_G metrics , Relativity_G RG_K1, Relativity_G RG_K2,  Relativity_G RG_K3, Relativity_G RG_K4, Relativity_G RG_K5, Relativity_G y_tilde_M, int s){
    ...
    if(id < Nr){
      ...
      if(id==0){
        //printf("A_tilda=%.15f | 1 : %.15f | 2 : %.15f | 3 : %.15f | 4 : %.15f | 5 : %.15f | 6 : %.15f \n",
        //y_tilde_M.A[6],y_tilde_M.A[7],y_tilde_M.A[8],y_tilde_M.A[9],y_tilde_M.A[10],y_tilde_M.A[11],y_tilde_M.A[12]);
        }
      if(id==0){
        y_tilde_M.Da[id]=0.0;
        y_tilde_M.Db[id]=0.0;
...
__global__ void next_step_metric(   Relativity_G metrics,  Relativity_G RG_K1,Relativity_G RG_K2, Relativity_G RG_K3, Relativity_G RG_K4, Relativity_G RG_K5){
  ...
  if(id<Nr){
    ...
    if(id==Nr-1 ){
      absorbente(metrics.A,0,id,1,  0);
      absorbente(metrics.B,0,id,1,0);
      absorbente(metrics.alpha,0,id,1, 0);
      absorbente(metrics.Da,0,id,1,  0);
      absorbente(metrics.Db,0,id,1,0);
      absorbente(metrics.K,0,id,1, 0);
      absorbente(metrics.Kb,0,id,1, 0);
      absorbente(metrics.U,0,id,1, 0);
      absorbente(metrics.lambda,0,id,1, 0);
    }
  }
  ...
  if(id < 6*dim_q){
    ...
    if(r==Nr-1 ){
      if(g ==0){
        ...
        c  = (dk*(k+1))/sqrt(dk*(k+1)*dk*(k+1) + 1.0);
      }
      else if(g==2 || g==4){
        c  = (dk*(k+1))/sqrt(dk*(k+1)*dk*(k+1) + 3.0);
      }
      else{
        c  = (dk*(k+1))/sqrt(dk*(k+1)*dk*(k+1) + 4.0);
      }
      absorbente(field_Q.real.u,id_nodo_ghost,r,c,  1);
      absorbente(field_Q.real.psi,id_nodo_ghost,r,c,1);
      absorbente(field_Q.real.pi,id_nodo_ghost,r,c, 1);
      absorbente(field_Q.imag.u,id_nodo_ghost,r,c,  1);
      absorbente(field_Q.imag.psi,id_nodo_ghost,r,c,1);
      absorbente(field_Q.imag.pi,id_nodo_ghost,r,c, 1);
...
__device__ double initial_A(double A, int i, double r, double cosmological_constant, int s){
 ...
 //double c_atrasada[3]={1.5, -2.0, 0.5};
  ...
  if (i < (int)(order-1)/2){
    ...
    dr_psi = (1.5*initial_psi(r) - 2.0*initial_psi(r-dr) + 0.5*initial_psi(r-2*dr));
    dr_psi /= (dr);
...
*/
    ...
    for (int t=0 ; t<Nt ;t++){
      printf("t : %d\n",t);
        ...
        cudaDeviceSynchronize();
        guardar_datos<<< grid_radial, bloque >>>(cuda_xphix,cuda_alpha,cuda_nodo,cuda_A,metrics, field_C,field_Q , t);
        cudaDeviceSynchronize();
