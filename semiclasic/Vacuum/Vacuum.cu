...
void derivate(double *df, double *f, double h, int N ){
  ...
  for(int idx=0; idx<N;idx++){
    ...
    if (idx<(int)(host_order-1)/2 ){
      for (int m=0;m<host_order;m++){
        temp += host_coefficient_adelantada[m]*f[idx+m];
      }
      df[idx]=temp/h; 
    }
    else if (idx > N-(int)(host_order-1)/2-1){
      for (int m=-host_order+1;m<1;m++){
      ...
      for (int m=0;m<host_order;m++){
        temp += host_coefficient_centrada[m]*f[idx-(int)(host_order-1)/2+m];
      }
      df[idx]=temp/h;
    }
  }
...
__device__ double PI_dot(double *chi, double *PI, double *A, double *B,double *alpha, double r2, int idx, int s, double dr, double dt, double *RK_C, int symmetric,int Nr,  int t, int error){
  ...
  if (idx < (int)(order-1)/2){
    ...
    for (int m=0; m < half_order_right + idx ; m++){
        ...
        temp += coefficient_centrada[m + (int)(order-1)/2 -idx ]*(f_PI_dot(chi, PI,A,B,alpha,RK_C, m ,s,dt,dr ,Nr , t , error));

      }
    for (int m=(int)(order-1)/2 - idx; m > 0 ; m--){

        temp += sym*coefficient_centrada[ (int)(order-1)/2 - idx - m  ]*(f_PI_dot(chi, PI,A,B,alpha,RK_C, m ,s,dt,dr ,Nr , t , error));
...
__device__ double chi_dot(double *PI, double *A, double *B,double *alpha,int idx, int s, double dr, double dt, double *RK_C, int symmetric,int Nr, int t, int error){
  ...
  if (idx < (int)(order-1)/2){
    ...
    for (int m=0; m < half_order_right + idx ; m++){
        ...
        temp += coefficient_centrada[m + (int)(order-1)/2 - idx ]*(f_chi_dot(PI,A,B,alpha,RK_C, m ,s,dt, Nr , t , error));

      }
    for (int m=(int)(order-1)/2 - idx; m > 0 ; m--){

        temp += sym*coefficient_centrada[ (int)(order-1)/2 - idx - m  ]*(f_chi_dot(PI,A,B,alpha,RK_C, m ,s,dt, Nr , t , error));
...
__device__ cufftDoubleComplex f_pi_dot(cufftDoubleComplex *u, cufftDoubleComplex *dr_u,double *A, double *B,double *alpha, double *lambda, double *RK_C, cufftDoubleComplex *RK_Q, 
  ...
  if(idr == Origin){
      f1 =  (l) *  (dr_u[g*space + id_nodo].x + dt*a_ij[s] * RK_Q[ g*3*space + space + id_nodo].x) ;
...
/*
  if(idr<32 && k ==19 && l ==3 &&g==0){
    printf("Z = %f | %d\n",Z.x,idr);


  }*/
  if(idr == Origin){
      f1 =  (l) *  (dr_u[g*space + id_nodo].y + dt*a_ij[s] * RK_Q[ g*3*space + space + id_nodo].y) ; //deberia ser cero
...
__global__ void fluctuation( cufftDoubleComplex *u, cufftDoubleComplex *u_p1, cufftDoubleComplex *dr_u, cufftDoubleComplex *pi, double *temp_array ,  double *temp_fluct, 
                              double dr, double dk, double dt, int Nr, int Nk , int Nl,int ghost, int l, int t ){
  int idx = threadIdx.x + blockDim.x*blockIdx.x;
  int id;
  double radio;
  //temp_fluct (5xNr)
...
__global__ void stress_energy(double *SA, double *SB, double *rho, double *ja, double *phi, double *chi, double *PI, double *A, double *B, double *alpha, double *temp_array , double dr, double dt, int Nr, int Nk, int Nl, int t){
  ...
  if(idx < Nr){
    ...
    espectation_dr_dt_phi = PI[idx]*chi[idx] +  temp_array[ 2*Nl*Nr + idx ];
    ...
    for (int s=0 ; s<4 ;s++){
      ...
      //chi0=0.01*(rs-5.0)*(-2.0/(0.2*0.2))*exp(-1.0*((rs-5.0)/0.2)*((rs-5.0)/0.2));
        ...
        temp=0.0;
            ...
            else if (idx > Nr-(int)(host_order-1)/2-1){
              for (int m=-host_order+1;m<1;m++){
              rs= dr*((idx-1)+m)+dr*host_c_i[s];
            ...
            for (int m=0;m<host_order;m++){
              rs= dr*((idx-1)-(int)(host_order-1)/2+m)+dr*host_c_i[s];
...
/////Quamtum field////
    for (int l = 0 ; l < Nl; l++ ){
      ...
      fluctuation<<< grid_radial  , bloque>>>( cuda_u_nodos, cuda_u_nodos_p1, cuda_dr_u, cuda_pi, temp_array, temp_fluct, dr, dk,dt, Nr, Nk, Nl, 0 ,l,t);
...
/////Ghosts fields////
    ...
    for (int g = 0 ; g<5 ; g++){
        for (int l = 0 ; l < Nl; l++ ){
            fluctuation<<< grid_radial , bloque >>>(cuda_u_ghost, cuda_u_ghost_p1, cuda_dr_u_ghost, cuda_pi_ghost, temp_array, temp_fluct, dr, dk, dt, Nr, Nk, Nl, g , l, t);
        }
        if (g==2 || g==0){
            mass = 0.0;
        }
