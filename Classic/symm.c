...
for(int t=1;t< Nt ;t++){
  ...
  for(int s=0 ; s < stage ; s++){
    for(int idx=0;idx<Nr;idx++){
      ...
      if(idx<Nr-1){
        K12[ (s+1)*Nr + idx ]=phi_evolution(PI[idx] + dt*a_ij[s]*K10[s*Nr+idx],A[idx] + dt*a_ij[s]*K9[s*Nr+idx],B[idx] + dt*a_ij[s]*K1[s*Nr+idx],alpha[idx] + dt*a_ij[s]*K3[s*Nr+idx],dt, Nr, t);
...
//if(t%10==0 && idx==1){
  //    printf("drK:%f",dr_Db);}
      ...
      //printf("kpi=%f\n",A[idx]);};
      if(s==3){
        B[idx]          =fabs(B[idx]+K1[idx]);
        Db[idx]         =Db[idx] +K2[idx];
        alpha[idx]      = fabs(alpha[idx]+K3[idx]);
        Da[idx]         =Da[idx]  + K4[idx];
        Kb[idx]         +=K5[idx];
        K[idx]          +=K6[idx];
        lambda[idx]     +=K7[idx];
        U[idx]          +=K8[idx];
        A[idx]          =fabs(A[idx] +K9[idx]);


        if (idx==Nr-1){
        PI[idx]=absorbente(PI,idx,dr,dt,Nr);
        chi[idx]=absorbente(chi,idx,dr,dt,Nr);
        temp_phi[idx]=absorbente(temp_phi,idx,dr,dt,Nr);
        }
        else{
    ...
    for(int idx=0;idx<Nr;idx++){ 
      ...
      //reinicio los K CEROS
        ...
        K5[idx]=0.0;K6[idx]=0.0;K7[idx]=0.0;K8[idx]=0.0;
        K9[idx]=0.0;K10[idx]=0.0;K11[idx]=0.0;K12[idx]=0.0;
        K[idx] =K[idx] - Kreiss_Oliger(K,idx,Nr,0.15);
        Kb[idx] =Kb[idx] - Kreiss_Oliger(Kb,idx,Nr,0.1);
      ...
      if(t%1==0){
        phi[(int)t*Nr/1 + idx] = temp_phi[idx] ;

        temp_Kb[(int)t*Nr/1 + idx] = Kb[idx] ;
        temp_alpha[(int)t*Nr/1 + idx] = alpha[idx] ;

      }

    if (idx==1 && t<1040 &&t>1020 ){
        printf("r : %d\n",idx);
...
guardar_salida_Kb(temp_Kb,Nr,Nt);
guardar_salida_alpha(temp_alpha,Nr,Nt);
